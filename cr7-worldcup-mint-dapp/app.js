(function () {
  ...
  const config = window.CR7_MINT_CONFIG || {};
  ...
  const TOKENS_PER_STEP = Number(config.TOKENS_PER_0_01_BNB || "10000");
  const TOKEN_SYMBOL = config.TOKEN_SYMBOL || "CR7";
  ...
  function storageKey(account) {
    return `cr7:bnb-minted:${(account || "").toLowerCase()}`;
  }
  ...
  function normalizeBnbInput(value) {
    ...
    if (parts.length > 1) {
      clean = `${parts[0]}.${parts.slice(1).join("").slice(0, 3)}`;
    }
    if (clean.startsWith(".")) {
      clean = `0${clean}`;
    }
  ...
  function updateProgress() {
    ...
    els.progressBlock.classList.toggle("progress-paused", !MINT_PROGRESS_ENABLED);
  ...
  function updateUi() {
    ...
    els.walletMintedBnb.textContent = formatBnb(state.walletMintedBnb);
    els.maxMintBnb.textContent = formatBnb(MAX_WALLET_MINT_BNB);
    ...
    if (!hasEthereum()) {
      ...
      setStatus("连接钱包后即可 mint，钱包确认后代币自动到账。", "neutral");
    } else if (!onBsc) {
      ...
      setStatus(`本次将到账 ${formatToken(getMintTokens(inputBnb))} ${TOKEN_SYMBOL}。`, "neutral");
    }
  ...
  async function refreshWalletMinted() {
    if (!state.account || !hasEthereum() || !isAddress(config.MINT_CONTRACT_ADDRESS) || !config.WALLET_MINTED_SELECTOR) {
      return;
    }
