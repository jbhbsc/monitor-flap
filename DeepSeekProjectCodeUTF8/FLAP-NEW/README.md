...
```text
质押/购买彩票代币: 0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777
奖池/中奖领取代币: 0xbe9d156892e55e7154bcd3cb0fea677f9d3103e1
```

当前规则是双币模式：

- 用户质押 `0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777` 购买彩票。
- 用户每质押 `10000` 枚质押代币获得 `1` 张刮刮乐彩票。
- 质押代币累计到 `500000` 枚后，奖池会把这 `500000` 枚质押代币全部打入黑洞地址。
- `0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777` 代币的营销税会产生 `0xbe9d156892e55e7154bcd3cb0fea677f9d3103e1`，营销钱包设置成奖池合约地址后，这部分 `0xbe9d` 会直接进入可派奖励余额。
...
注意：普通 ERC20 转账不会自动执行接收方合约逻辑。因此 `0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777` 的营销税产生 `0xbe9d156892e55e7154bcd3cb0fea677f9d3103e1` 并进入奖池时，不需要也不会自动卖出；它本身就是中奖领取代币，会直接计入 `rewardBalance()`。
...
- 每轮 `10` 分钟。
- 不需要凑满 `50` 张才开奖；本轮只要有 `1` 个编号，10 分钟结束后就可以开奖。
...
- `contracts/FlapPrizePool.sol`：外部奖池合约，负责处理质押代币打黑洞和奖励发放，也作为 `0x42D8` 营销税产出 `0xbe9d` 的接收钱包。
...
1. 准备质押/购票代币：
   ...
   ```text
   0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777
   ```
...
4. 部署 `FlapPrizePool`。
   ...
   ```text
   prizeToken_       = 0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777
...
5. 部署 `FlapScratchLottery`。
   ...
   ```text
   stakeToken_       = 0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777
...
7. 回到 flap 平台，把 `0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777` 代币合约的营销钱包改成 `FlapPrizePool` 地址。这个营销税产生并转入奖池的是 `0xbe9d156892e55e7154bcd3cb0fea677f9d3103e1`。
...
不要直接把 `0x42D8055eB359DD7F19f6eE3fAEBf45eA9d237777` 转入奖池合约；普通 ERC20 直接转账不能自动执行卖出逻辑。自动打黑洞由用户通过刮刮乐彩票合约购买时触发。
